...
class RetryBudget:
	...
	def can_retry(self, service: str) -> bool:
		...
		if recent_retries >= self.max_retries_per_minute:
			...
			logger.warning(
				f'[RetryBudget] {service}: {recent_retries} retries in window, entering {self.cooldown_seconds}s cooldown'
			)
			return False
		...
		if total >= 10:  # need enough data for ratio to be meaningful
			...
			if ratio > self.max_retry_ratio:
				stats.cooldown_until = now + self.cooldown_seconds
				logger.warning(
					f'[RetryBudget] {service}: retry ratio {ratio:.2%} exceeds {self.max_retry_ratio:.0%}, entering cooldown'
				)
				return False
	...
	def get_all_health(self) -> Dict[str, Any]:
		...
		return {service: self.get_health(service) for service in self._services}
	...
	def stats(self) -> Dict[str, Any]:
		...
		return {
			...
			'services_in_cooldown': [s for s, stats in self._services.items() if time.time() < stats.cooldown_until],
		}
