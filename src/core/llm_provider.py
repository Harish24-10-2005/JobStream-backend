...
class UnifiedLLM:
	...
	def __init__(self, temperature: float = 0.3, max_retries: int = 3, retry_delay: float = 1.0):
	...
	def _create_llm(self, config: LLMConfig):
		...
		if config.provider == LLMProvider.GROQ:
			...
			return ChatGroq(model=config.model, temperature=config.temperature, api_key=config.api_key)
	...
	def invoke(self, messages: List[Dict[str, str]], agent_name: str = '') -> str:
		...
		# Economic guardrail: block agent calls when daily budget is exhausted.
		if agent_name:
			try:
				if not cost_tracker.check_budget(agent_name):
					raise LLMError(f'Budget exhausted for agent: {agent_name}')
			except LLMError:
				raise
			except Exception as e:
				logger.warning(f'Budget guardrail check failed (fail-open) for {agent_name}: {e}')
