...
class Console:
	...
	def header(self, title: str, icon: str = ''):
		...
		print()
		print(self._colorize(f'{prefix}{title} ', Colors.BOLD + Colors.BRIGHT_YELLOW) + self._colorize(line, Colors.DIM))
	...
	def step(self, number: int, total: int, message: str):
		...
		self._safe_print(self._colorize(f'  {progress} ', Colors.BRIGHT_MAGENTA) + self._colorize(message, Colors.WHITE))
	...
	def box(self, title: str, content: List[str], color: str = Colors.CYAN, icon: str = ''):
		...
		# Bottom border
		print(self._colorize(f'{self.BOX_BOTTOM_LEFT}{self.BOX_HORIZONTAL * (inner_width + 2)}{self.BOX_BOTTOM_RIGHT}', color))

	def status_box(self, agent_name: str, status: str, details: Dict[str, Any], color: str = Colors.CYAN, icon: str = '🤖'):
		"""Print a status box for an agent."""
		content = [
			self._colorize('Status: ', Colors.DIM) + self._colorize(status, Colors.BRIGHT_WHITE),
		]
		...
		for key, value in details.items():
			...
			content.append(self._colorize(f'{key}: ', Colors.DIM) + self._colorize(str(value), Colors.WHITE))
	...
	def progress_bar(self, current: int, total: int, label: str = '', width: int = 40):
		...
		print(self._colorize(f'  {bar} ', Colors.DIM) + self._colorize(f'{percentage}%', color))
	...
	def score_display(self, score: int, label: str = 'Match Score'):
		...
		print(
			self._colorize(f'  {label}: ', Colors.DIM)
	...
	def scout_results(self, query: str, location: str, urls: List[str]):
		...
		for i, url in enumerate(urls[:10], 1):  # Show max 10
			...
			path = (
				'/'.join(url.split('/')[3:])[:30] + '...'
				if len('/'.join(url.split('/')[3:])) > 30
				else '/'.join(url.split('/')[3:])
			)
	...
	def analyst_results(
		...
		# Job info box
		content = [
			self._colorize(f'{self.SYMBOL_USER} Role: ', Colors.DIM) + self._colorize(role, Colors.BOLD + Colors.BRIGHT_WHITE),
			self._colorize(f'{self.SYMBOL_COMPANY} Company: ', Colors.DIM) + self._colorize(company, Colors.BRIGHT_CYAN),
			self._colorize(f'{self.SYMBOL_MONEY} Salary: ', Colors.DIM) + self._colorize(salary, Colors.BRIGHT_GREEN),
		]
		...
		if tech_stack:
			...
			content.append(
				self._colorize(f'{self.SYMBOL_SKILLS} Tech: ', Colors.DIM) + self._colorize(tech_display, Colors.BRIGHT_MAGENTA)
			)
		...
		if analysis:
			print(
				self._colorize(f'  {self.SYMBOL_BULLET} Analysis: ', Colors.DIM)
	...
	def applier_header(self, url: str):
		...
		print(self._colorize('  Starting browser automation for application...', Colors.DIM))

	def applier_status(self, status: str, detail: str = ''):
		"""Print ApplierAgent status update."""
		timestamp = datetime.now().strftime('%H:%M:%S')
		print(
			self._colorize(f'  [{timestamp}] ', Colors.DIM)
	...
	def workflow_start(self, query: str, location: str):
		...
		print(self._colorize(f'    {self.SYMBOL_SEARCH} Query:    ', Colors.DIM) + self._colorize(query, Colors.BRIGHT_WHITE))
		print(self._colorize('    🌍 Location: ', Colors.DIM) + self._colorize(location, Colors.BRIGHT_WHITE))
		print(
			self._colorize(f'    {self.SYMBOL_CLOCK} Started:  ', Colors.DIM)
	...
	def workflow_job_progress(self, current: int, total: int, url: str):
		...
		domain = url.split('/')[2] if len(url.split('/')) > 2 else url
		print(
			self._colorize(f'  {self.SYMBOL_TARGET} Processing Job ', Colors.BOLD)
	...
	def workflow_skip(self, reason: str, company: str, role: str, score: int):
		...
		print(
			self._colorize(f'  {self.SYMBOL_SKIP} Skipping: ', Colors.YELLOW)
	...
	def workflow_match(self, company: str, role: str, score: int):
		...
		print(
			self._colorize(f'  {self.SYMBOL_MATCH} MATCH FOUND! ', Colors.BOLD + Colors.BRIGHT_GREEN)
			+ self._colorize(f'{company} - {role}', Colors.WHITE)
	...
	def workflow_summary(self, total_jobs: int, analyzed: int, applied: int, skipped: int):
		...
		print(
			self._colorize(f'  {self.SYMBOL_CLOCK} Completed: ', Colors.DIM)
