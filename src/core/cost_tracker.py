...
class CostTracker:
	...
	def record(
		...
		logger.debug(f'[CostTracker] {agent_name} | {provider}/{model} | in={input_tokens} out={output_tokens} | ${cost:.6f}')
	...
	def check_budget(self, agent_name: str) -> bool:
		...
		if current_spend >= agent_budget:
			logger.warning(f'[CostTracker] Budget exceeded for {agent_name}: ${current_spend:.4f} >= ${agent_budget:.4f}')
			return False
		return True
