...
class InvocationContext:
    ...
    def record(self, result):
        ...
        if hasattr(result, "usage_metadata") and result.usage_metadata:
            ...
            self._usage.input_tokens = getattr(meta, "input_tokens", 0) or meta.get("input_tokens", 0) if isinstance(meta, dict) else getattr(meta, "input_tokens", 0)
            self._usage.output_tokens = getattr(meta, "output_tokens", 0) or meta.get("output_tokens", 0) if isinstance(meta, dict) else getattr(meta, "output_tokens", 0)
