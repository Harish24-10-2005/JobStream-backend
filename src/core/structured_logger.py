...
class StructuredLogger:
    ...
    def agent_error(self, agent_name: str, action: str, error: str, **kwargs):
        ...
        entry = self._build_entry(
            "error", "agent", action, agent=agent_name, error=error, **kwargs
        )
    ...
    def llm_error(self, provider: str, model: str, error: str, **kwargs):
        """Log an LLM failure."""
        entry = self._build_entry(
            "error", "llm", "error", provider=provider, model=model, error=error, **kwargs
        )
        self._emit("error", entry)
    ...
    def pipeline(self, step: str, status: str, duration_ms: float = 0, **kwargs):
        ...
        entry = self._build_entry(
            "info", "pipeline", step, status=status, duration_ms=round(duration_ms, 1), **kwargs
        )
        self._emit("info", entry)
            ...
            def __call__(self, func):
                ...
                async def wrapper(*args, **kwargs):
                    ...
                    try:
                        ...
                        slog.custom(
                            category, action,
                            status="success", duration_ms=round(elapsed, 1),
                        )
                        return result
