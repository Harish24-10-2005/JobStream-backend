...
class AgentMemory:
	...
	async def remember(
		...
		if self._client:
			try:
				...
				self._client.table('agent_memories').upsert(data, on_conflict='user_id,agent_name,memory_key').execute()
	...
	async def forget(self, agent_name: str, user_id: str, key: str) -> bool:
		...
		if self._client:
			try:
				self._client.table('agent_memories').delete().eq('agent_name', agent_name).eq('user_id', user_id).eq(
					'memory_key', key
				).execute()
				return True
			except Exception as e:
	...
	async def record_feedback(
		...
		if self._client:
			try:
				...
				logger.info(f'[Memory] Feedback recorded: {agent_name} rated {rating}/5 by user {user_id[:8]}...')
				return True
			except Exception as e:
	...
	async def get_feedback_summary(
		...
		try:
			query = self._client.table('agent_feedback').select('rating, comments, created_at').eq('agent_name', agent_name)
			if user_id:
	...
	async def get_learnings(
		...
		"""
		memories = await self.recall_all(agent_name, user_id or 'system', MemoryType.LEARNING)
		...
		# Legacy data shape support: some callers/tests store "content" records.
		if not learnings and self._client:
			try:
				query = self._client.table('agent_memories').select('content').eq('agent_type', agent_name).eq('user_id', user_id or 'system')
				result = query.order('created_at', desc=True).limit(limit or 50).execute()
				legacy = [row.get('content') for row in (result.data or []) if row.get('content')]
				learnings.extend(legacy)
			except Exception:
				pass
		...
		if feedback['total'] > 0:
			if feedback['avg_rating'] < 3.0:
				...
				learnings.append(f'User is highly satisfied ({feedback["avg_rating"]}/5). Continue current approach.')

		return learnings

	async def store(self, agent_name: str, user_id: str, content: str) -> bool:
		"""Backward-compatible alias used by older tests/callers."""
		try:
			self._ensure_client()
			if not self._client:
				return await self.remember(agent_name, user_id, f'legacy:{datetime.now(timezone.utc).timestamp()}', content)
			...
			data = {
				...
				'user_id': user_id,
				'content': content,
				'memory_key': f'legacy:{datetime.now(timezone.utc).timestamp()}',
