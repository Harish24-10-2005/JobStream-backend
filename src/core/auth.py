...
def require_auth(request: Request) -> AuthUser:
	...
	if not auth_header or not auth_header.startswith('Bearer '):
		raise HTTPException(
			status_code=401, detail='Missing or invalid Authorization header', headers={'WWW-Authenticate': 'Bearer'}
		)
...
class RateLimitByUser:
	...
	def check_rate_limit(self, user_id: str) -> bool:
		...
		self.user_requests[user_id] = [ts for ts in self.user_requests[user_id] if ts > minute_ago]
