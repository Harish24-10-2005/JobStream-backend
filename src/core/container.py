...
class Container:
    ...
    def register_singleton(
        self,
        name: str,
        factory_or_class: Union[Callable, Type],
        *args,
        **kwargs
    ) -> "Container":
        """
        ...
        self._registrations[name] = ServiceRegistration(
            name=name, factory=factory, scope=LifecycleScope.SINGLETON
        )
    ...
    def register_factory(
        ...
        """
        self._registrations[name] = ServiceRegistration(
            name=name, factory=factory, scope=LifecycleScope.FACTORY
        )
    ...
    def register_instance(self, name: str, instance: Any) -> "Container":
        ...
        reg = ServiceRegistration(
            name=name, factory=lambda: instance, scope=LifecycleScope.SINGLETON
        )
    ...
    def resolve(self, name: str) -> Any:
        ...
        if name not in self._registrations:
            raise KeyError(
                f"Service '{name}' not registered. Available: {list(self._registrations.keys())}"
            )
    ...
    def health_check(self) -> Dict[str, str]:
        ...
        return {
            name: "initialized" if reg.initialized else "pending"
