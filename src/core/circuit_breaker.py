...
class CircuitBreaker:
            ...
            try:
                ...
                if attempt < attempts - 1:
                    ...
                    if retry_budget and not retry_budget.can_retry(self.name):
                        logger.warning(
                            f"Circuit {self.name}: retry blocked by RetryBudget (storm prevention)"
                        )
                        break

                    delay = self.retry_delay * (self.retry_backoff ** attempt)
                    logger.info(
                        f"Circuit {self.name}: retry {attempt + 1}/{self.retry_count} "
                        f"in {delay:.1f}s ({type(e).__name__})"
                    )
    ...
    def _transition_to_open(self):
        ...
        self.metrics.state_changes.append({
            "from": old_state.value, "to": "OPEN", "time": time.time()
        })
        logger.warning(
    ...
    def _transition_to_half_open(self):
        ...
        self.metrics.state_changes.append({
            "from": old_state.value, "to": "HALF_OPEN", "time": time.time()
        })
    ...
    def _transition_to_closed(self):
        ...
        self.metrics.consecutive_failures = 0
        self.metrics.state_changes.append({
            "from": old_state.value, "to": "CLOSED", "time": time.time()
        })
