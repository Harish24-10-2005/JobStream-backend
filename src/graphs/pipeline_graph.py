...
class PipelineCheckpoint:
	...
	def save(self, state: dict, completed_node: str) -> None:
		...
		safe['job_results'] = [r.model_dump() if hasattr(r, 'model_dump') else dict(r) for r in results]
	...
	def load(self) -> Optional[dict]:
		...
		try:
			with open(self._path, 'r', encoding='utf-8') as f:
				data = json.load(f)
			logger.info(f"Checkpoint restored from '{data.get('_checkpoint_node')}' ({data.get('_checkpoint_ts')})")
			return data
		except Exception as exc:
...
async def tailor_resume_node(state: dict) -> dict:
	...
	try:
		...
		events_update = _add_event(
			...
			'resume:complete',
			'resume',
			'Resume tailored' if path else 'Tailoring skipped',
...
async def cover_letter_node(state: dict) -> dict:
	...
	events_update = _add_event(state, 'cover_letter:start', 'cover_letter', 'Drafting cover letter...')
...
async def run_pipeline_graph(
	...
	async for step in graph.astream(initial_state, stream_mode='updates'):
		...
		for node_name, node_output in step.items():
			...
			logger.info(
				f"[Graph] Node '{node_name}' completed. Status: {node_output.get('node_statuses', {}).get(node_name, 'unknown')}"
			)
