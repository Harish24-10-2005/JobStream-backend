...
from src.api.schemas import ResumeDownloadResponse, ResumeTailorResponse, ResumeTemplatesResponse
...
async def analyze_resume(request: ResumeAnalyzeRequest, current_user: Annotated[AuthUser, Depends(get_current_user)]):
	"""
...
Be specific and actionable with suggestions. Focus on real improvements."""
		...
		llm = ChatGroq(model='llama-3.1-8b-instant', api_key=settings.groq_api_key.get_secret_value(), max_tokens=2048)
		...
		if json_match:
			...
			result = {
				...
				'suggestions': [
					{
						'type': 'improvement',
						'text': 'Add quantifiable achievements to your experience section',
						'priority': 'high',
					},
		...
		return ResumeAnalyzeResponse(
			...
			suggestions=[ATSSuggestion(**s) for s in result.get('suggestions', [])],
		...
		return ResumeAnalyzeResponse(
			...
			ats_score=70,
			metrics=ATSMetrics(
				keywords_found=10, keywords_total=15, format_status='Good', length_status='Good', contact_complete=True
			),
			suggestions=[
...
async def tailor_resume(request: ResumeTailorRequest, current_user: Annotated[AuthUser, Depends(get_current_user)]):
	"""
...
async def get_resume_history(current_user: Annotated[AuthUser, Depends(get_current_user)], limit: int = 20):
...
async def download_resume(resume_id: str, current_user: Annotated[AuthUser, Depends(get_current_user)]):
