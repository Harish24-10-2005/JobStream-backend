...
from src.api.schemas import JobAnalyzeResponse, JobApplyResponse, JobSearchResponse
...
async def search_jobs(request: JobSearchRequest, user: Annotated[AuthUser, Depends(get_current_user)]):
	"""
...
async def get_job_results(
	...
	try:
		result = db_service.get_jobs_with_analyses(
			limit=limit, offset=offset, min_score=min_score, source=source, user_id=user.id
		)
...
@router.get('/{job_id}')
async def get_job(job_id: str, user: Annotated[AuthUser, Depends(get_current_user)]):
	"""
...
async def analyze_job(job_id: str, user: Annotated[AuthUser, Depends(get_current_user)]):
	"""
...
async def apply_to_job(job_id: str, user: Annotated[AuthUser, Depends(get_current_user)], trigger_agent: Annotated[bool, Query()] = False):
	"""
