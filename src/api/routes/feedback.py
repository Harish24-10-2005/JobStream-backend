...
class ApplicationOutcomeRequest(BaseModel):
    ...
    outcome: str = Field(
        ...,
        description="interview_scheduled, rejected, offer_received, ghosted, withdrawn"
    )
...
@router.post("/salary")
async def submit_salary_feedback(
    req: FeedbackRequest,
    user: AuthUser = Depends(get_current_user),
):
    """Rate salary negotiation advice."""
    await agent_memory.record_feedback(
    ...
    return {"status": "recorded", "message": "Salary advice feedback saved."}
...
async def get_feedback_summary(
    ...
    valid_agents = [
        ...
        "salary_agent", "company_agent", "scout_agent",
    ]
