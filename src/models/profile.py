...
class NetworkMatch(BaseModel):
	...
	connection_type: Literal['alumni', 'location', 'company', 'mutual'] = Field(..., description='How they connect to the user')
...
class SkillGap(BaseModel):
	...
	learning_resources: List[LearningResource] = Field(default_factory=list, description='Curated learning resources')
	estimated_hours: float = Field(default=10.0, description='Total hours to reach competency')
	suggested_project: str = Field(..., description='Project idea to demonstrate this skill on resume')
...
class GapAnalysis(BaseModel):
	...
	priority_order: List[str] = Field(default_factory=list, description='Skills in order of priority to learn')
