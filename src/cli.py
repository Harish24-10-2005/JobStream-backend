...
Examples:
  ...
  python -m src.cli track --add "Google" "SWE"
	...
	# ============================================
	search_parser = subparsers.add_parser('search', help='🔍 Search and apply to jobs (full pipeline)')
	search_parser.add_argument('query', help='Job title/keywords')
	search_parser.add_argument('location', help="Location (e.g., 'Remote', 'NYC')")
	search_parser.add_argument('--min-score', type=int, default=70, help='Minimum match score (default: 70)')
	search_parser.add_argument('--no-resume', action='store_true', help='Skip resume tailoring')
	search_parser.add_argument('--no-cover', action='store_true', help='Skip cover letter generation')

	# ============================================
	# INTERVIEW - Interview prep
	# ============================================
	interview_parser = subparsers.add_parser('interview', help='🎯 Prepare for interviews')
	interview_parser.add_argument('role', help='Job role')
	interview_parser.add_argument('company', help='Company name')
	interview_parser.add_argument('--tech', default='', help='Tech stack (comma-separated)')

	# ============================================
	# COMPANY - Company research
	# ============================================
	company_parser = subparsers.add_parser('company', help='🏢 Research companies')
	company_parser.add_argument('name', help='Company name')
	company_parser.add_argument('--role', default='', help='Target role for context')

	# ============================================
	# RESUME - Resume tailoring
	# ============================================
	resume_parser = subparsers.add_parser('resume', help='📄 Tailor resume for a job')
	resume_parser.add_argument('role', help='Job role')
	resume_parser.add_argument('company', help='Company name')
	resume_parser.add_argument('--description', default='', help='Job description text')

	# ============================================
	# TRACK - Application tracker
	# ============================================
	track_parser = subparsers.add_parser('track', help='📊 Manage job applications')
	track_parser.add_argument('--report', action='store_true', help='Show application report')
	track_parser.add_argument('--add', nargs=2, metavar=('COMPANY', 'ROLE'), help='Add new application')
	track_parser.add_argument('--update', nargs=2, metavar=('COMPANY', 'STATUS'), help='Update application status')
	track_parser.add_argument('--list', dest='list_apps', action='store_true', help='List all applications')
...
async def run_search(args):
	...
	workflow = JobApplicationWorkflow(use_resume_tailoring=not args.no_resume, use_cover_letter=not args.no_cover)
