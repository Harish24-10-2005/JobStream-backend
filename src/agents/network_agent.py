...
from src.models.profile import NetworkMatch, NetworkSearchResult, UserProfile
...
class NetworkAgent(BaseAgent):
	...
	async def _generate_outreach(self, match: NetworkMatch, user_profile: UserProfile, company: str) -> str:
		...
		try:
			...
			personal = getattr(user_profile, 'personal_information', None)
			user_name = getattr(personal, 'first_name', None) or getattr(user_profile, 'first_name', 'there')
			user_college = user_profile.education[0].university if getattr(user_profile, 'education', None) else 'my university'
			user_title = user_profile.experience[0].title if getattr(user_profile, 'experience', None) else 'Software Engineer'
			...
			try:
				...
				if learnings:
					...
					company_learnings = [learning for learning in learnings if company.lower() in learning.lower()]
			...
			common_ground = []
			if match.connection_type == 'alumni' and getattr(match, 'college_match', None):
				common_ground.append(f'We are both alumni of {match.college_match}. Go {str(match.college_match).split()[0]}!')
			...
			user_skills = set()
			for skill_list in getattr(user_profile, 'skills', {}).values():
...
"""
			...
			return (
				f"Hi {match.name}, I see we have some shared background and I'd love to connect to follow your work at {company}."
			)
	...
	async def find_connections(
		...
		result = NetworkSearchResult(company=company, search_query='', search_source='serpapi')
		...
		# Calculate totals
		result.total_matches = len(result.alumni_matches) + len(result.location_matches) + len(result.company_matches)

		# Generate outreach messages for top matches
		if generate_outreach:
			# Prioritize: alumni > location > company
			top_matches = result.alumni_matches[:3] + result.location_matches[:2] + result.company_matches[:2]

			for match in top_matches:
				if not match.outreach_draft:
					match.outreach_draft = await self._generate_outreach(match, user_profile, company)
	...
	async def run(self, company: str, user_profile: Optional[UserProfile] = None, **kwargs) -> AgentResponse:
		"""
		...
		try:
			if user_profile is None:
				return AgentResponse.create_error('user_profile is required')
