...
def extract_job_requirements(
	role: str, company: str, tech_stack: List[str], matching_skills: List[str] = None, missing_skills: List[str] = None
) -> Dict:
	"""
...
def tailor_resume_content(profile_json: str, requirements_json: str, feedback: str = '', rag_context: str = '') -> AgentResponse:
	"""
	...
	compact_profile = {
		...
		'experience': [
			{'title': exp.get('title', ''), 'company': exp.get('company', '')} for exp in profile.get('experience', [])[:2]
		],
	}
...
def generate_latex_resume(tailored_content_json: str, template_type: str = 'ats') -> str:
	"""
	...
	if template:
		latex = resume_service.fill_template(template['latex_content'], tailored, tailored)
...
def calculate_ats_score(tailored_content_json: str, tech_stack: List[str]) -> int:
	"""
	...
	score = resume_service.calculate_ats_score(tailored, {'tech_stack': tech_stack})
...
class ResumeAgent(BaseAgent):
	...
	async def run(self, *args, **kwargs) -> Dict:
		...
		# Legacy contract support used by older tests/callers.
		if kwargs.get('resume_text') and kwargs.get('job_description'):
			try:
				llm = get_llm()
				resume_text = kwargs.get('resume_text', '')
				job_desc = kwargs.get('job_description', '')
				tone = kwargs.get('tone', 'Professional')
				resume_out = await llm.ainvoke(
					[{'role': 'user', 'content': f'Tailor this resume in {tone} tone.\n\nRESUME:\n{resume_text}\n\nJOB:\n{job_desc}'}],
					agent_name='resume_agent',
				)
				analysis = await llm.agenerate_json(
					prompt=f'Analyze ATS alignment for this resume and job.\n\nRESUME:\n{resume_text}\n\nJOB:\n{job_desc}',
					agent_name='resume_agent',
				)
				if isinstance(analysis, dict) and 'error' in analysis:
					return AgentResponse.create_error(analysis['error'])
				return AgentResponse.create_success(
					data={
						'resume': resume_out,
						'ats_score': analysis.get('ats_score', 0) if isinstance(analysis, dict) else 0,
	...
	async def tailor_resume(
				...
				if isinstance(val, str) and val:
					...
					for item in val:
						if isinstance(item, str):
							...
							if check.is_blocked:
								slog.agent_error(
									'resume_agent', f'guardrail_blocked_{dict_name}_{key}_item', {'reason': check.blocked_reason}
								)
								return AgentResponse.create_error(
		...
		try:
			...
			console.step(4, 6, 'Generating LaTeX document')
			latex_result = generate_latex_resume(tailored_content_json=tailored_json, template_type=template_type)

			# Step 4: Calculate ATS score
			ats_score = calculate_ats_score(tailored_content_json=tailored_json, tech_stack=job_data.get('tech_stack', []))
	...
	async def generate_pdf(self, latex_source: str, output_path: str = None) -> Optional[str]:
