...
class PDFService:
    ...
    def generate_company_dossier(self, data: Dict, filename: str) -> str:
        ...
        story.append(Paragraph(f"CONFIDENTIAL INSIDER DOSSIER", self.styles["Normal"]))
        ...
        culture_text.append(f"<b>Work-Life Balance:</b> {culture.get('work_life_balance', {}).get('rating', 'N/A')} - {culture.get('work_life_balance', {}).get('notes', '')}")
