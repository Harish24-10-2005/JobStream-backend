...
from .exceptions import LLMUnavailableError, ProfileNotFoundError
from .llm_config import get_llm_client
from .models import AgentConfig, ApplierResult, ApplierStatus, Timeouts
...
class LiveApplierService:
	...
	async def take_screenshot(self):
		...
		if screenshot:
			await self.emit(EventType.BROWSER_SCREENSHOT, 'Manual screenshot', {'screenshot': screenshot, 'format': 'jpeg'})
	...
	def _is_llm_error(self, e: Exception) -> bool:
		...
		return 'ModelProviderError' in error_type or 'ModelRateLimitError' in error_type or '429' in error_str
		...
		try:
			if self._browser_manager._browser_session:
				...
				await tracker_agent.add_application(
					company=url, role='Software Engineer', url=url, notes='Auto-applied via Live Applier.', priority='High'
				)
		except Exception as e:
			logger.error(f'Failed to track application: {e}')

	def _create_result(self, success: bool, url: str, message: str = '', error: str = None) -> ApplierResult:
		...
		return ApplierResult(
			success=success, status=self._status, url=url, message=message, error=error, duration_seconds=duration
		)
