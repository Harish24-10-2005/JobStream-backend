...
def create_applier_tools(
	...
	async def ask_human(question: str) -> ActionResult:
		...
		await emit(EventType.HITL_REQUEST, question, {'hitl_id': hitl_id, 'context': 'Form requires your input'})
	...
	async def retrieve_user_context(query: str) -> ActionResult:
		...
		try:
			...
			if results:
				formatted = '\n\n'.join([f'- {r.get("content", r)}' if isinstance(r, dict) else f'- {r}' for r in results])
