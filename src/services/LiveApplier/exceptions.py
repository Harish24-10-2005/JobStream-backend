...
class ProfileNotFoundError(LiveApplierError):
	...
	def __init__(self, user_id: str, message: str = None):
		super().__init__(message or f'Profile not found for user: {user_id}', details={'user_id': user_id})
...
class LLMUnavailableError(LiveApplierError):
	...
	def __init__(self, attempted_providers: list = None):
		...
		super().__init__(
			f'No LLM available. Attempted: {", ".join(providers) or "None"}', details={'attempted_providers': providers}
		)
...
class NavigationError(LiveApplierError):
	...
	def __init__(self, url: str, reason: str = None):
		super().__init__(f'Failed to navigate to: {url}', details={'url': url, 'reason': reason})
		self.url = url
