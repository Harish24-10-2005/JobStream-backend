...
class CredentialService:
    ...
    async def store_credential(
        ...
        response = supabase_client.table("platform_credentials").upsert(
            data,
    ...
    async def get_credential(
        ...
        """
        response = supabase_client.table("platform_credentials").select(
            "encrypted_username, encrypted_password, encryption_iv, is_valid"
        ).eq("user_id", user_id).eq("platform", platform.lower()).single().execute()
        ...
        supabase_client.table("platform_credentials").update(
            {"last_used": "now()"}
        ).eq("user_id", user_id).eq("platform", platform.lower()).execute()
    ...
    async def delete_credential(self, user_id: str, platform: str) -> bool:
        ...
        response = supabase_client.table("platform_credentials").delete().eq(
            "user_id", user_id
    ...
    async def invalidate_credential(self, user_id: str, platform: str) -> bool:
        ...
        response = supabase_client.table("platform_credentials").update(
            {"is_valid": False}
        ).eq("user_id", user_id).eq("platform", platform.lower()).execute()
    ...
    async def list_platforms(self, user_id: str) -> list:
        ...
        response = supabase_client.table("platform_credentials").select(
            "platform, credential_type, is_valid, last_used, created_at"
