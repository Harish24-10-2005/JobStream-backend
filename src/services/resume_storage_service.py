...
class ResumeStorageService:
    ...
    async def get_user_resumes(self, user_id: str) -> List[ResumeMetadata]:
        ...
        try:
            response = self.client.table(self.TABLE_USER_RESUMES) \
                .select("*") \
                .eq("user_id", user_id) \
    ...
    async def get_generated_resumes(
        ...
        user_id: str,
        limit: int = 50
    ) -> List[GeneratedResumeMetadata]:
        """Get user's generated resumes history."""
        try:
            response = self.client.table(self.TABLE_GENERATED_RESUMES) \
                .select("*") \
                .eq("user_id", user_id) \
                .order("created_at", desc=True) \
                .limit(limit) \
            ...
            for r in response.data or []:
                results.append(GeneratedResumeMetadata(
                    id=r.get("id"),
...
Return ONLY the JSON object, no explanations."""
    ...
    async def get_cover_letters(
        ...
        try:
            response = self.client.table(self.TABLE_COVER_LETTERS) \
                .select("*") \
                .eq("user_id", user_id) \
                .order("created_at", desc=True) \
                .limit(limit) \
