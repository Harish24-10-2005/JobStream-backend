...
class DatabaseService:
	...
	def get_discovered_jobs(
		self, limit: int = 20, offset: int = 0, source: Optional[str] = None, user_id: Optional[str] = None
	) -> Dict[str, Any]:
		"""
		...
		try:
			...
			return {'jobs': result.data or [], 'total': result.count or 0, 'offset': offset, 'limit': limit}
	...
	def get_jobs_with_analyses(
		...
		try:
			...
			return {'jobs': jobs, 'total': jobs_result.count or 0, 'offset': offset, 'limit': limit}
	...
	def search_jobs(self, query: str = '', location: str = '', limit: int = 20) -> List[Dict]:
	...
	def save_cover_letter(
		...
		try:
			...
			if result.data:
				console.success('Cover letter saved')
				return letter_id
	...
	def get_applications_summary(self, user_id: Optional[str] = None) -> Dict:
		...
		try:
			...
			return {'total': len(result.data), 'by_status': by_status, 'applications': result.data}
