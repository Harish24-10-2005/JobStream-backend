...
async def update_profile(
    ...
    try:
        ...
        if not success:
            raise HTTPException(
                status_code=404, 
                detail="Profile not found. Please create a profile first using POST /api/user/profile"
            )
...
async def get_profile_completion(user: AuthUser = Depends(get_current_user)):
    """
...
async def add_education(
    ...
    education_id = await user_profile_service.add_education(
        user_id=user.id,
        education=request.model_dump()
    )
...
async def add_experience(
    ...
    experience_id = await user_profile_service.add_experience(
        user_id=user.id,
        experience=request.model_dump()
    )
...
async def add_project(
    ...
    """Add a project entry."""
    project_id = await user_profile_service.add_project(
        user_id=user.id,
